# Contributing to ep_ai_mcp

Thanks for helping improve `ep_ai_mcp` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_ai_mcp` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_ai_mcp from your local clone
pnpm run plugins i --path ../ep_ai_mcp

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_ai_mcp/` are picked up after a server restart.
...
* Etherpad version, ep_ai_mcp version, browser + OS
* Steps to reproduce
